...
which contains a Data Element Dictionary, this flag "quality control data."
...
far easier to drop the laboratory duplicates.

The "D" values appear to often occur in strings of multiple observations
...
SUR'.  Almost all have depth = 0.2.

```{r}
dep_data %>%
  filter(`QC Type` == 'D') %>%
  select(- site_name, -dt, - Time, -month, -year, -hour, -`Sample Type`, 
         - `Depth Unit`, -`QC Type`, - `Sampled By`) %>%
  select(- c(temp:chl_a_sonde)) %>%
  select(-c(secchi:irr_pct)) %>%
...
FOCB do not look like familiar sampling codes, so we will need to check GIS locations
...
We see that site_name encompasses in a few cases multiple annotated 
`Sample Point ID`s.  In particular, `Sample Point ID`s are sometimes decorated
with " - SUR", " - BOT", and " - MAX", apparently for "Surface", "Bottom", and 
"Maximum" samples respectively.  One sample location, at Royal River, has the 
annotation "_A" instead.
...
We use regex to extract non-numeric data and look at what we find.

```{r}
tmp <- dep_data %>%
  # delete teh QA samples
  filter(`QC Type` == 'NA')
...
We use a regex to identify digits, spaces, minus signs, and decimals.  We use
`gsub()` to replace them all with the empty string.  For some reason, this 
also appears to pick up commas, although we don't call for that directly.
...
This suggests we should pull out a separate data series for the irradiance
values.
...
So we can assemble a tibble with that assumption.  We also check if we have 
any duplicate data, which we don't.
