...
behind.   However,a t these sites, data was often only collected a few times, making seasonal analysis problematic.
...
State of Casco Bay. Given space constraints, our primary use of these
data is likely to be as  examples showing vertical profiles in a side bar.  
...
strong at most sites. Temperatures change by a degree C every couple of meters or so.  This is enough to be meaningful, and slow vertical mixing, but this is not
evidence of string thermal stratification as occurs in Maine lakes.

Reverse thermal stratification occurs sometimes on the Presumpscot Estuary 
sites, presumably because of salinity gradients that stabilize thermal patterns.


profile exists 
EEB18 also shows weak vertical thermal structure of a degree or two
...
It might be worth looking at patterns headed down the Presumpscot Estuary,a s follows:
```{r}
